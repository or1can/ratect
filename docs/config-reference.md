...
| `image_pull_policy` | `IfNotPresent` or `Always` | no | Whether to pull an `image` container's image fresh or only when it's missing locally. Defaults to `IfNotPresent`, matching Batect. Only meaningful alongside `image` — Ratect doesn't implement Batect's separate use of this same field to force-pull a `build_directory` build's base image (see [Differences from Batect](differences-from-batect.md#container-fields)). No expression support. |
