...
| `--docker-cert-path`, `--docker-tls`, `--docker-tls-verify`, `--docker-tls-ca-cert`, `--docker-tls-cert`, `--docker-tls-key` | Not supported | Deferred — see `ROADMAP.md`'s 0.17.0 entry. Connecting over TLS needs a `rustls` crypto provider installed process-wide, and bollard's `connect_with_ssl` has no way to skip server-certificate verification while still doing client-cert auth, unlike Batect's `--docker-tls` (without `-verify`). |
