...
| `--clean`, `--clean-cache` | Not supported | Would clear out this project's existing cache volumes/directories and exit. Not a build-performance feature either way (the Docker build cache itself is unaffected) — these govern Batect's own cache *volumes*, a distinct mechanism. |
