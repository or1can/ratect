...
| `capabilities_to_add` / `capabilities_to_drop` | Supported | Validated against exactly the capability names Batect itself supports (its `Capability` enum, based on `capabilities(7)`) — an unknown name is rejected at config-load time rather than reaching Docker's API to fail there. Container level only, matching Batect. No expression support. |
