...
| `--output <STYLE>` | `-o` | auto | Forces a particular output style for Ratect's own progress reporting (never affects the task command's own output): `fancy` (a live-updating status block, one line per container), `simple` (plain, append-only milestone lines), or `quiet` (error messages only, and a machine-readable `--list-tasks` format) — see [Output styles](#output-styles). `all` is an accepted name but not implemented yet (it arrives later in 0.16.0 — requesting it errors clearly). Unset means auto-select: `fancy` on an interactive console, `simple` otherwise. |
...
- **`all`** — an accepted name (so scripts written against the final 0.16.0
  surface parse today) but not implemented yet; requesting it errors clearly
