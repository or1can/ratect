...
| `--output <STYLE>` | `-o` | auto | Forces a particular output style for Ratect's own progress reporting (never affects the task command's own output): `simple` (plain, append-only milestone lines, no live progress detail) or `quiet` (error messages only, and a machine-readable `--list-tasks` format — see [Output styles](#output-styles)). `fancy` and `all` are accepted names but not implemented yet (they arrive later in 0.16.0 — requesting one errors clearly). Unset means auto-select: `simple` on any non-interactive console; `fancy` once it exists on an interactive one. |
...
styles are Batect's own four, two of which are implemented so far:
...
- **`fancy`**, **`all`** — accepted names (so scripts written against the final
  0.16.0 surface parse today) but not implemented yet; requesting either errors
  clearly rather than silently falling back.

When `--output` isn't given, Ratect auto-selects: `simple` on any non-interactive
console (stdout not a terminal, `TERM` unset or `dumb`, terminal size unqueryable,
or `--no-color` given); `fancy`, once implemented, on an interactive one — until
then the interactive default is `simple` too. `quiet` and `all` are never
auto-selected.
