...
- **`ratect-core`** (library crate, `ratect-core/src/`): all the reusable logic, with
  ...
  | `resources.rs` | What previous runs left behind and how it's removed — `ratect resources list`/`clean`'s selection and removal, generic over `ContainerRuntime` |
...
9.  **ROADMAP.md / RELEASES.md Maintenance**: a `~~strikethrough~~` marking completed scope is
    **inline markdown and cannot cross a blank line** — a multi-paragraph entry
    ...
    balanced; GitHub's own `/markdown` API answers it in one call. Beyond that, the two files follow different edit rules, which is why they *are* two files. `ROADMAP.md` is forward-looking and its `## Batect Parity` headline list is a living summary — freely edit, merge, or delete bullets as scope changes or ships (e.g. "Sidecar Containers" and "Docker Networking" were merged into "Full Docker Networking" once shipped). [`RELEASES.md`](RELEASES.md) holds the per-binary versioned lists, and they are append-only history — never delete an entry; mark completed scope with `~~strikethrough~~` plus a done-summary of what actually shipped. They were one file until the history reached 75% of it, at which point a document named "roadmap" was mostly a record of the past; `ROADMAP.md` keeps stub headings at the two anchors the lists used to occupy, so links written before the split — including ones in already-released `CHANGELOG.md` sections, which are append-only — still resolve.
