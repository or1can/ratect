...
7a. **Version Lifecycle**: When cutting a release or starting the next version's development, follow the `X.Y.Z-dev` ↔ `X.Y.Z` bump convention documented in [ROADMAP.md](ROADMAP.md#versioning--releases) — don't skip it, and don't fold it into a feature commit.
