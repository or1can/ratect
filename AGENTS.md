...
- **`ratect-core`** (library crate, `ratect-core/src/`): all the reusable logic, with
  ...
  - **`ratect-core/src/config.rs`**: Data models for the configuration (`batect.yml`).
    ...
    errors. `PortRange`/`PortMapping` have hand-written `Deserialize` impls (a
    `serde::de::Visitor` calling `deserializer.deserialize_any`, since `noyalib`
    supports it) accepting either Batect form for a `ports` entry — a
    `"local:container[/protocol]"` string (`PortMapping::parse_string`) or an object
    (`{local, container, protocol}`, `PortRange` itself taking either a bare int or a
    `"from-to"` string) — validating `local`/`container` cover the same number of
    ports at construction time either way. `PortMapping::expand` turns a (possibly
    ranged) mapping into concrete `(local_port, container_port, protocol)` triples,
    zipped by position for a range.
  ...
  - **`ratect-core/src/docker.rs`**: A wrapper around the `bollard` library. It manages
    ...
    to its own container name. `build_port_config` is a pure function turning
    already-expanded, already-validated `(local_port, container_port, protocol)`
    triples (`config::PortMapping::expand` — parsing/validation itself lives in
    `config.rs`, not here) into `Config.exposed_ports`/`HostConfig.port_bindings`.
  ...
  - **`ratect-core/src/engine.rs`**: The core execution logic. It manages the task
    ...
    `merged_ports` expands and concatenates a container's own `ports` with a task
    run's *additional* `ports` (a union, not an override — matching how Batect
    combines these as a `Set`) before `docker.rs` ever sees them, so
    `NetworkOptions::ports` only ever carries already-resolved
    `(local_port, container_port, protocol)` triples.
