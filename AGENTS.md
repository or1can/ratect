...
- **`ratect-core`** (library crate, `ratect-core/src/`): all the reusable logic, with
  ...
  | `registry_auth.rs` | Resolving private registry credentials via real Docker credential helpers — the `RegistryCredentialResolver` trait, its `DockerCredentialHelperResolver` production implementation, and registry-hostname extraction. A filesystem-and-subprocess concern kept out of `docker.rs`, which wires it into `pull_image` via `resolve_pull_credential`. Build-time resolution is a later addition |
