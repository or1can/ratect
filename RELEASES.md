...
`ROADMAP.md` keeps stub headings at the two anchors this file's sections used
to live at, so links written before the split still resolve.
...
- **0.4.0** — ~~**Interactive Mode** (TTY/STDIN attachment for tasks that need user
  ...
  - Windows terminal handling (raw mode, resize) is implemented via `crossterm`
    ...
    Support](ROADMAP.md#rust-enhancements) not having started yet.
...
- **0.15.0** — ~~**Parallel Task Execution**: independent prerequisites and tasks run
  ...
  - `prerequisites` staying strictly sequential is a deliberate scope decision, not a
    ...
    Batect for later (see [Rust Enhancements](ROADMAP.md#rust-enhancements)), not committed to.
...
- **0.20.0** — **Two-Binary Split**: ~~the workspace actually splits as described
  in [Two Binaries](ROADMAP.md#two-binaries-ratect-and-ratect-compat), as its own
...
- **0.21.0** (planned) — **Parity Mop-Up II**: closes known Batect-parity gaps
  ...
  - ~~**Config Schema**: a JSON schema for `batect.yml`'s actual Ratect-accepted
    ...
    the [Batect Parity](ROADMAP.md#batect-parity) headline entry for the full reasoning
...
- **0.25.0** — **Cleanup on interrupt, `build_ssh` parity, and the next
  ...
  - ~~**`build_ssh` full parity** — multiple named agents and, the practically
    ...
    Parity](ROADMAP.md#batect-parity) field tables, so it's on the 1.0.0 critical path
  ...
  - ~~**More of Batect's journey corpus** — the [conformance phase](#ratect-compat)
    ...
    [First-class Cross-platform Support](ROADMAP.md#rust-enhancements) starts, which is not
...
- **0.26.0** — **Cleanup on any termination signal, and proxies that
  ...
  [`ratect doctor`'s](ROADMAP.md#ux--tooling) list — a proxy variable that isn't a URL or
...
- **0.27.0** — Beyond what the refactoring release below (see [`ratect`
  0.6.0](#ratect)) reaches it through `ratect-core` — none of which changes
  this binary's own surface — three real fixes: **pulling or building from a
  private registry now resolves credentials the way the `docker` CLI does**
  (`~/.docker/config.json`'s `auths`/`credsStore`/`credHelpers`, including
  keychain-backed stores and cloud registries via their credential helper);
  **a Docker daemon isn't rejected just for being older than the very latest
  release** — the API version is now negotiated against the real daemon, down
  to a 20.10 floor; and **a TLS connection to the daemon no longer fails
  outright when a single OS trust-store entry can't be read** (patched in the
  `bollard` fork, offered upstream as
  [fussybeaver/bollard#796](https://github.com/fussybeaver/bollard/pull/796)).

  Its parity surface is otherwise unchanged by design, and one commit exists
  specifically to keep it that way: `--clean-cache`'s Batect-verbatim output
...
- **0.1.0-dev** — ~~a placeholder crate exists (`ratect/`, added alongside
  `ratect-compat` in 0.20.0's [Two-Binary Split](ROADMAP.md#two-binaries-ratect-and-ratect-compat))
...
- **0.2.0** — ~~**CLI subcommand skeleton**: `ratect run <task>` and
  ...
  [UX & Tooling](ROADMAP.md#ux--tooling)) have somewhere to live without a breaking
...
- **0.4.0** — **Include trust and sharing**: picks up where 0.24.0's
  ...
  - ~~**`allow_nested_git_includes`** (defaulting `false`) — the [Future
    Vision](ROADMAP.md#future-vision) item: today a bundle reached through a Git include
...
- **0.5.0** — **nothing of its own; it ships
  ...
  identical.
- **0.6.0** — **A refactoring release**, in two parts. First, six candidates
  from an architecture review run after 0.26.0 shipped — each grilled before
  anything was written, each independently landable — gave four homeless
  concepts a module or seam of their own (leftover/`doctor` findings, cache
  storage, Docker connection setup, container runtime specs), plus a
  shared-classification fix (`include_trust::gate`, unifying how the loader
  and shell completion decide whether a Git include may be read) and a
  `TaskEngine` construction cleanup. Then, immediately after, four smaller
  unifications in the same spirit: `ConfigFormat`'s dialect-conditional rules
  into one place, `ContainerRuntime` split by caller
  (`ResourceInventory`/`VolumeStore`), a `BuildSpec` counterpart to
  `ContainerSpec`, and `Boundary`'s containment and grants halves reunited in
  `include_trust.rs`. No user-visible behaviour changed anywhere in either
  part — every commit's own message says so, verified against the real
  Docker suite where relevant. See the closed [ratect-compat 0.27.0 · ratect
